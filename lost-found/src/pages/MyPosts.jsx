...
export default function MyPosts() {
  ...
  useEffect(() => {
    Promise.allSettled([
      axios.get("https://lost-found-app-4-w6wh.onrender.com/api/items/mine"),
      axios.get("https://lost-found-app-4-w6wh.onrender.com/api/claims/mine"),
    ]).then(([ir, cr]) => {
  ...
  const handleItemResolve = async (comment) => {
    ...
    try {
      await axios.patch(`https://lost-found-app-4-w6wh.onrender.com/api/items/${item._id}/resolve`, { resolveComment: comment })
  ...
  const handleClaimResolve = async (comment) => {
    ...
    try {
      await axios.patch(`https://lost-found-app-4-w6wh.onrender.com/api/claims/${claim._id}/resolve`, { resolveComment: comment })
  ...
  const handleClaimDelete = async () => {
    try {
      await axios.delete(`https://lost-found-app-4-w6wh.onrender.com/api/claims/${deleteTarget._id}`)
  ...
  const handleClaimEditSave = async (fields) => {
    try {
      const res = await axios.patch(`https://lost-found-app-4-w6wh.onrender.com/api/claims/${editClaim._id}`, fields)
              ...
              {items.map(item => {
                ...
                return (
                  <div style={S.card} key={item._id}
                    ...
                    {item.images?.length > 0
                      ? <img src={`https://lost-found-app-4-w6wh.onrender.com/uploads/${item.images[0]}`} alt="" style={S.cardImg} />
                ...
                return (
                  <div style={S.card} key={claim._id}>
                    {claim.images?.length > 0
                      ? <img src={`https://lost-found-app-4-w6wh.onrender.com/uploads/${claim.images[0]}`} alt="" style={S.cardImg} />
